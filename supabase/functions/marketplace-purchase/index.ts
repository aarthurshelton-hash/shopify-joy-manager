...
serve(async (req) => {
  ...
  try {
    ...
    // Database-level idempotency: check if already sold to this buyer
    const { data: alreadySold } = await supabaseClient
    ...
    if (alreadySold) {
      logStep("Already purchased by this user", { listingId });
      return new Response(JSON.stringify({ 
        success: true, 
        message: "Already purchased",
        visualizationId: alreadySold.visualization_id,
        duplicate: true 
      }), {
