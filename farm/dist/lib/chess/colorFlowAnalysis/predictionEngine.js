...
function describeExpectedEvolution(signature, currentMove) {
    ...
    if (currentMove < 15) {
        ...
        return `Middlegame ${archetype.name} pattern established. Color flow is ${signature.dominantSide === 'contested' ? 'evenly contested' : `favoring ${signature.dominantSide}`}. Watch for tactical breaks in the ${signature.flowDirection}.`;
    }
    else {
        return `Late game ${archetype.name}. Pattern suggests ${signature.dominantSide === 'contested' ? 'drawing tendencies' :
